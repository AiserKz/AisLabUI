...
const AnimationShowcaseItem = React.memo(function AnimationShowcaseItem({ item }: { item: any }) {
  ...
  const card = (
    <ShowcaseCard
      ...
      icon={isVideo ? <Play className="w-6 h-6 text-primary" /> : <Layers className="w-6 h-6 text-primary" />}
...
export default function Animations() {
  ...
  return (
    <div className="w-full justify-center flex flex-col lg:flex-row relative gap-6 px-4 md:px-8 pb-20">
      <Aside
        sections={animationsData.map(s => ({ id: s.id, title: s.title, length: s.items.length }))}
        ...
        {animationsData.map((section) => (
          <Section key={section.id} id={section.id}>
            <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
                ...
                {section.title}
                <span className="text-base-content/30 text-sm font-normal ml-auto">{section.items.length} items</span>
            </h2>
